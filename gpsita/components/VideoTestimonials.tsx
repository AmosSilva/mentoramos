...
const VideoTestimonialCard: React.FC<VideoTestimonial> = ({ name, title, youtubeId }: VideoTestimonial) => (
  <div className="bg-[#131f25] p-8 rounded-2xl border-2 border-[#192a44] flex flex-col text-left h-full shadow-[-4px_-4px_100px_-8px_rgba(0,64,153,0.5),_4px_4px_100px_-8px_rgba(0,122,82,0.6)]">
          ...
          <div className="flex gap-2">
              {videoTestimonialData.map((_, index: number) => (
                  <button
                      ...
                      className={`w-3 h-3 rounded-full transition-all ${activeDotIndex === index ? 'bg-[#ffe566]' : 'bg-white/20 hover:brightness-125 hover:shadow-[inset_0_0_0_1px_rgba(255,255,255,0.7)]'}`}
