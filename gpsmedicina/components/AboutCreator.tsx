...
import {
  ...
  academicTimelineData,
} from "../src/data/timelineData";
...
const AboutCreator: React.FC = () => {
  ...
  const { targetRef: gifTargetRef, isIntersecting } = useIntersectionObserver({
    threshold: 0.5,
  });
  ...
  const timelineData =
    activeTimeline === "professional"
      ? professionalTimelineData
      : academicTimelineData;

  // Effect to check for 2xl screen size
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (container && activeItem && !is2xl) {
      // Only scroll if not 2xl
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    if (!containerRef.current || is2xl) return; // Disable drag on 2xl
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    if (!isDragging.current || !containerRef.current || is2xl) return; // Disable drag on 2xl
    e.preventDefault();
    const x = e.pageX - containerRef.current.offsetLeft;
    const walk = x - startX.current;
    if (Math.abs(walk) > 5) {
      // Threshold to prevent click from being a drag
    ...
    <div className="flex justify-center gap-8 mb-8">
      <button
        ...
        className={`pb-2 font-semibold transition-colors duration-300 ${
          activeTimeline === "professional"
            ? "text-white border-b-2 border-[#7fffd4]"
  ...
  return (
    <section
      id="about-creator"
      className="py-20 my-20 bg-[#131f25] shadow-[-4px_-4px_100px_-8px_rgba(0,64,153,0.5),_4px_4px_100px_-8px_rgba(0,122,82,0.6)]"
    >
      <div className="container mx-auto px-6 xl:px-28">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <div className="text-gray-300 leading-relaxed space-y-4 mt-8 text-center lg:text-left">
              <p>
                Textinho bacana Textinho bacana Textinho bacana Textinho bacana
                Textinho bacana Textinho bacana Textinho bacana Textinho bacana
                Textinho bacana{" "}
              </p>
            </div>
        ...
        <div className="mt-20">
          ...
          <div
            ref={containerRef}
            className="relative overflow-x-auto scrollbar-hide -mx-6 xl:-mx-28 cursor-grab 2xl:overflow-visible 2xl:mx-0 2xl:cursor-default"
          ...
          >
            <div
              className="relative flex items-start pl-6 xl:pl-28 pt-4 2xl:justify-center 2xl:px-0"
              style={!is2xl ? { paddingRight: "calc(50vw - 80px)" } : {}}
            >
              {/* The connecting line */}
              <div
                className="absolute top-[12.5rem] h-px bg-white/30 left-[104px] xl:left-[192px] 2xl:left-1/2 2xl:-translate-x-1/2"
                style={{
              ...
              {timelineData.map((item, index) => (
                ...
                >
                  <div
                    className={`w-32 h-32 rounded-lg mb-4 mx-auto overflow-hidden transition-all duration-300 transform group-hover:scale-125 ${
                  ...
                  <p
                    className={`font-bold font-manrope text-lg transition-colors group-hover:text-white ${
                  ...
                  <div className="mt-4 relative h-6">
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                      <div
                        className={`w-4 h-4 bg-[#131f25] border-2 rounded-full relative z-10 transition-all group-hover:border-[#7fffd4] group-hover:bg-[#7fffd4] ${
                          activeIndex === index
                            ? "border-[#7fffd4] bg-[#7fffd4]"
                            : "border-white/10"
                        }`}
                      ></div>
                    </div>
