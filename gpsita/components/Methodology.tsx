...
import { methodologyData } from '../src/data/methodologyData';

const MethodologyItem: React.FC<{ icon: React.ReactNode; title: string; description: string; isLast: boolean }> = ({ icon, title, description, isLast }) => (
  <div className="group flex items-stretch gap-6">
    <div className="relative">
      <div className="w-16 h-16 bg-[#0c1b2d] border-2 border-[#3c555d] rounded-full flex items-center justify-center flex-shrink-0 transition-shadow duration-300 shadow-[-1px_-1px_4px_0px_rgba(0,64,153,0.5),_1px_1px_4px_0px_rgba(0,122,82,0.6)] group-hover:shadow-[-4px_-4px_50px_0px_rgba(0,64,153,0.5),_4px_4px_50px_0px_rgba(0,122,82,0.6)]">
        {icon}
      </div>
      {!isLast && (
